...
def update_tailscale():
    ...
    if remote_version > local_version:
        # set build version 
        build["args"]["TAILSCALE_VERSION"] = remote_version
        ...
        new_version += ".0" * (3 - new_version.count("."))

        print(f"upgrade required, new version: {new_version}")
        ...
        call_all_or_fail(
            ["git", "add", config_path],
            ["git" "add", build_path],
            ["git", "commit", "-m", f"updated tailscale to {new_version}"]
        )
